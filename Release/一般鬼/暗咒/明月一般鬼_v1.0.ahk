...
MsgBox "Ming is runing! Press F2 to start~"
F2::
...
loop{   
    ...
    loop{
        sleep 2000
    }Until ( Getcolor(1skill[1],1skill[2],1) )  ;(GetColor(triangleInside[1],triangleInside[2],1))   ; 进门后右上角地图角标
        send, {w down}
        sleep 20000
...
Fight(ele){
    ...
    if ( ele == 1 || ele == 2){
        ...
        if (ele == 1){
            ...
            }until ( timer > 100  || Getcolor(dieskill[1],dieskill[2],1) || GetColor(xskill[1],xskill[2],1) )    ; 灰色2+4  或者死4  ( GetColor(2Greyskill[1],2Greyskill[2],1) &&  )
        }else{
            send {1}
            sleep 800
            send {tab}
            ...
            }until ( GetColor(xskill[1],xskill[2],1)  || Getcolor(dieskill[1],dieskill[2],1) )   ; z  或者死4  ( GetColor(2Greyskill[1],2Greyskill[2],1) &&  )
        }
        ...
        ; }
            ...
            loop{
                ...
                send r
                timer++
        }until ( (GetColor(2Greyskill[1],2Greyskill[2],1) && GetColor(xskill[1],xskill[2],1)) || Getcolor(dieskill[1],dieskill[2],1) || GetColor(missionboxInside[1],missionboxInside[2],1) )   ; 灰色2+4  或者死4 || 箱子
    }
